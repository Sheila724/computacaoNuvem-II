...
const projectId = process.env.PUBSUB_PROJECT_ID || process.env.PROJECT_ID;
const credentialsPath = process.env.GOOGLE_APPLICATION_CREDENTIALS || path.join(__dirname, 'service-account-key.json');
const topicName = process.env.PUBSUB_TOPIC || process.env.TOPIC_NAME || 'pedidos-topic';

if (!projectId) {
  throw new Error('PUBSUB_PROJECT_ID (ou PROJECT_ID) nao configurado.');
}
